...
class MyWindow(QtWidgets.QMainWindow):
    ...
    def changeComPort1(self):
        global comport1
        comport1 = self.ui.listWidget.currentItem().text()
        self.ui.label_7.setText(comport1)
        self.pump1 = longer.Longer(comport1)
    ...
    def start1(self):
        global running1
        if not running1 and comport1 is not None:
            try:
                running1 = True
                self.pump1.setSettings(state=True)
                self.pump1.printToCom(self.pump1.getWriteMsg())
            except serial.SerialException:
                self.handle_error('could not open port ' + comport1)

    @pyqtSlot()
    def stop1(self):
        global running1
        if running1 and comport1 is not None:
            try:
                running1 = False
    ...
    def start2(self):
        global running2
        if not running2 and comport2 is not None:
            running2 = True
            self.pump2.setSettings(state=True)
            try:
                self.pump2.printToCom(self.pump2.getWriteMsg())
            except serial.SerialException:
                self.handle_error('could not open port ' + comport2)

    @pyqtSlot()
    def stop2(self):
        global running2
        if running2 and comport2 is not None:
            try:
                running2 = False
